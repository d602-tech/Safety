...
// The goal is to remove the BACKSLASH from \` and \${ and }
// These were introduced by the previous "fix" script
...
// 3. Fix manage-grid (L1357 approx)
...
// Apply replacements
content = content.replace(/(?s)getProgressHtml: \(c\) => \{.*?\},/, progressHtml);
content = content.replace(/(?s)getFileStatusHtml: \(c\) => \{.*?\},/, fileStatusHtml);
content = content.replace(/(?s)<div class="manage-grid">.*?<\/div>/, manageGridHtml);

const buttonsRegex = /\\\$\{\(c\['.*?'\] \|\| c\['.*?'\] \|\| c\['.*?'\] \|\| c\['.*?'\] \|\| c\['.*?'\]\).*?<\/div>\\\` : ''\}/;
content = content.replace(buttonsRegex, buttonsBlockHtml);

// Global cleanup: search for any occurrence of \\` or \\${ and replace with ` or ${
// (But only in our targeted sections to avoid side effects)
// Actually the templates above already fixed it for those sections.

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully fixed app.js formatting and labels');
